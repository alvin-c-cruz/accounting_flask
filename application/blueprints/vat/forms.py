...
from .models import Vat as Obj
from .models import UserVat as Preparer
...
class Form:
    ...
    def _validate_on_submit(self):
        ...
        if not self.vat_name:
            self.errors["vat_name"] = "Please type vat description."
        else:
            existing_ = Obj.query.filter(func.lower(Obj.vat_name) == func.lower(self.vat_name), Obj.id != self.id).first()
            if existing_:
                self.errors["vat_name"] = "Vat description already exists."
