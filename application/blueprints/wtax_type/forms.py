...
from .models import WtaxType as Obj
from .models import UserWtaxType as Preparer
...
class Form:
    ...
    def _validate_on_submit(self):
        ...
        if not self.wtax_type_name:
            self.errors["wtax_type_name"] = "Please encode wtax type."
        else:
            existing_ = Obj.query.filter(func.lower(Obj.wtax_type_name) == func.lower(self.wtax_type_name), Obj.id != self.id).first()
            if existing_:
                self.errors["wtax_type_name"] = "Wtax type already exists."
