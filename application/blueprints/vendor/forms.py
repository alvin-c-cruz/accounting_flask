...
class Form:
    ...
    def _validate_on_submit(self):
        ...
        if not self.vendor_name:
            ...
            if existing_:
                self.errors["vendor_name"] = "Vendor name already exists."

        if self.tin:
            existing_ = Obj.query.filter(func.lower(Obj.tin) == func.lower(self.tin), Obj.id != self.id).first()
            if existing_:
                self.errors["tin"] = "Tax Identification Number already exists."
